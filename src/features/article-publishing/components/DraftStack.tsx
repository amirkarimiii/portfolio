...
export interface DraftStackProps {
    searchParams: {
        page?: string;
        sort?: string;
    };
...
export async function DraftStack({ searchParams, pageSize = 20 }: DraftStackProps) {
    const sortOrder = searchParams.sort === 'oldest' ? 'oldest' : 'newest';

    const { articles, totalPages, currentPage } = await ArticleService.getDraftArticles({
            ...
            <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b pb-6">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                        Manage and edit article drafts
                    </p>
                </div>
            ...
            {articles.length === 0 ? (
                ...
                <section className="flex flex-col gap-4 items-center">
                    {articles.map((article) => (
                        <ContentCard origin="draft" key={article.uniqueId} type="article" data={article} />
                    ))}
                </section>
            )}

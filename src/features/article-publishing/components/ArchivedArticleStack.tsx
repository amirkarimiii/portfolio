...
export interface ArchivedArticleStackProps {
    searchParams: {
        page?: string;
        sort?: string;
    };
...
export async function ArchivedArticleStack({ searchParams, pageSize = 20 }: ArchivedArticleStackProps) {
    const sortOrder = searchParams.sort === 'oldest' ? 'oldest' : 'newest';

    const { articles, totalPages, currentPage } = await ArticleService.getArchivedArticles({
    ...
    return (
        <>
            <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b pb-6">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">Archived Articles</h1>
                    <p className="text-sm text-muted-foreground mt-1">
                        View and restore archived articles
                    </p>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex border rounded-lg overflow-hidden p-0.5 bg-muted/20">
                        ...
                        >
                            <Link
                                href={{
                                    pathname: '/admin/articles/archive',
            ...
            {articles.length === 0 ? (
                ...
                <section className="flex flex-col gap-4 items-center">
                    {articles.map((article) => (
                        <ContentCard origin="archive" key={article.uniqueId} type="article" data={article} />
                    ))}
                </section>
            )}
